...
from .flags import IFlag, MappingConfig, FlagRenderer

class MapEntry(TypedDict):
	id: int
	flags: frozenset[tuple[str, Any]]
	blocks: _BlockCollection
    ...
    def solve(self, show_progress: bool = True):
        ...
        with Progress(transient=True) as progress:
            ...
            for index, (flag_name, flag) in enumerate(self.flags.items()):
                ...
                for subflag, subflag_blocks in flag.atomic_flags.items():
                    new_flag = (flag_name, True) if subflag is True else (flag_name, subflag)
                    generators.append([
                        (frozenset([new_flag]), subflag_blocks.difference, encountered)
                    ])
                    generators.append([
...
class BlockMapping:
    ...
    def render_decoder(self):
        ...
        for flag_name, flag in self.flags.items():
            values: dict[Any, list[int]] = dict()
            for entry in self.mapping:
                for k, v in entry["flags"]:
                    if k == flag_name:
                        values.setdefault(v, list()).append(entry["id"])
            
            lines.extend(flag.render_decoder(values, self.config))
            lines.append('')
            lines.append('')
